...
var regex = /^[a-zA-Z]*$/;
  if((regex.test(fname)&&fname.length>3)){
  ...
  console.log("valid");
 if(regex.test(sname) && sname.length<=3) {
  ...
  if((regex.test(hname) && hname.length>3) && hname.length<=3) {
  ...
  if(regex.test(lname) && lname.length<=3) {
